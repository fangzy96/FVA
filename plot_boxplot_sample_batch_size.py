...
def find_sample_files(dataset: str, prefix: str = "", model_dir: str = None) -> Dict[str, Path]:
    """Find sample files for dataset. prefix e.g. llada15_, model_dir overrides global."""
    ...
    for b in batches:
        ...
        if len(files) == 0:
            continue
        out[b] = files[-1]
    _debug(f"dataset={dataset}, prefix={prefix}, found files for batches={list(out.keys())}")
    return out

# -------------------- TARGET INDEX (acc datasets) --------------------
...
def load_acc_rows(dataset: str, batch: str, path: Path) -> pd.DataFrame:
    rows = []
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            best_other = max([lls[j] for j in range(n) if j != target_idx]) if n > 1 else ll_true
...
def build_tables(prefix: str = "", model_dir: str = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Build acc and pass tables. prefix e.g. llada15_, model_dir overrides global."""
...
def plot_margin_hist(acc_df, dataset, outdir):
    """Overlay margin distribution by batch with transparent colors."""
    sub = acc_df[acc_df["dataset"] == dataset].copy()
    if sub.empty:
        return

    present = list(sub["batch"].dropna().unique())
    ordered_batches = [b for b in batches if b in present] + [b for b in present if b not in batches]
    ...
    bins = 40
    palette = sns.color_palette("tab10", n_colors=max(3, len(ordered_batches)))

    for i, b in enumerate(ordered_batches):
        vals = sub[sub["batch"] == b]["margin"].dropna().values
        if vals.size == 0:
            continue
        sns.histplot(
            x=vals,
    ...
    plt.title(f"Margin distribution by batch ({dataset})")
    plt.xlabel("Log-likelihood margin (true - best_other)")
    plt.ylabel("Density")
    plt.legend(loc="upper right", frameon=True, fontsize=9)
    plt.tight_layout()

    outfile = Path(outdir) / f"margin_hist_{dataset}_by_batch.png"
    fig.savefig(outfile, dpi=300)
    plt.close(fig)
    print(f"[OK] Saved overlaid margin histogram for {dataset} → {outfile}")


# ----------------------- Process single model -----------------------
def process_model(prefix: str = "", model_dir: str = None, out_root: Path = None, model_name: str = ""):
    """Process one model. prefix e.g. llada15_, out_root output dir, model_name for filenames."""
    if out_root is None:
        out_root = Path("figs_samplewise_all")
    if model_name:
    ...
    if not acc_df.empty:
        ...
        for ds in acc_datasets:
            plot_margin_hist(acc_df, ds, out_root)
...
def calculate_flip_rate_for_model(ds: str, prefix: str, model_dir: str, model_name: str, is_acc: bool):
    """Prediction Flip Rate: FlipRate_pred(i) = 1 - max_a(#{c: pred_{i,c}=a}) / |C|."""
    files = find_sample_files(ds, prefix=prefix, model_dir=model_dir)
    if not files:
        return None
    
    all_batch_data = {}
    for batch, file_path in files.items():
        if is_acc:
            df = load_acc_rows(ds, batch, file_path)
            if not df.empty:
                all_batch_data[batch] = df[["doc_id", "pred"]]
        else:
            df = load_pass_rows(ds, batch, file_path)
            if not df.empty:
                all_batch_data[batch] = df[["doc_id", "pass_flag"]]
    
    if not all_batch_data:
        return None
    
    common_doc_ids = set(all_batch_data[list(all_batch_data.keys())[0]]["doc_id"])
    for batch_df in all_batch_data.values():
        common_doc_ids &= set(batch_df["doc_id"])
    
    if not common_doc_ids:
        return None
    
    flip_rates = []
    n_configs = len(all_batch_data)
    
    for doc_id in common_doc_ids:
        pred_list = []
        for batch, batch_df in all_batch_data.items():
            row = batch_df[batch_df["doc_id"] == doc_id]
            if not row.empty:
                if is_acc:
...
def calculate_flip_rate():
    """Compute Prediction Flip Rate for llada and llada15 across datasets (by batch settings)."""
    print("\n" + "="*80)
    print("Prediction Flip Rate (LLaDA and LLaDA-1.5, by batch settings)")
    print("="*80)
    ...
    if results:
        ...
        out_dir = Path("figs_samplewise_all")
        out_dir.mkdir(parents=True, exist_ok=True)
        results_df.to_csv(out_dir / "flip_rate_by_batch.csv", index=False)
        print(f"\nSaved: {out_dir / 'flip_rate_by_batch.csv'}")
...
def plot_flip_rate_barplot(results_df: pd.DataFrame, out_dir: Path):
    """Plot flip rate bar chart (ICML-style)."""
    ...
    for i, model in enumerate(models):
        ...
        error_color = colors[i] if i == 0 else colors[i]
...
# ----------------------- Main -----------------------
def main():
    base_out_root = Path("figs_samplewise_all")
